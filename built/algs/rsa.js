...
var Rsa = (function (_super) {
            ...
            session.generateKeyPair(graphene_pk11_1.KeyGenMechanism.RSA, template.publicKey, template.privateKey, function (err, keys) {
                try {
                    if (err)
                        ...
                        var wcKeyPair = {
                            privateKey: new key_1.CryptoKey(keys.privateKey, _alg_1),
                            publicKey: new key_1.CryptoKey(keys.publicKey, _alg_1)
                        };
    ...
    Rsa.exportJwkPublicKey = function (session, key, callback) {
        try {
            ...
            var jwk = {
                ...
                ext: true,
                key_ops: key.usages,
                e: utils_1.Base64Url.encode(pkey.publicExponent),
                n: utils_1.Base64Url.encode(pkey.modulus)
            };
            callback(null, jwk);
        }
        catch (e) {
    ...
    Rsa.exportJwkPrivateKey = function (session, key, callback) {
        try {
            ...
            var jwk = {
                ...
                e: utils_1.Base64Url.encode(pkey.publicExponent),
                n: utils_1.Base64Url.encode(pkey.modulus),
                d: utils_1.Base64Url.encode(pkey.privateExponent),
                p: utils_1.Base64Url.encode(pkey.prime1),
                q: utils_1.Base64Url.encode(pkey.prime2),
                dp: utils_1.Base64Url.encode(pkey.exp1),
                dq: utils_1.Base64Url.encode(pkey.exp2),
                qi: utils_1.Base64Url.encode(pkey.coefficient)
            };
            callback(null, jwk);
        }
        catch (e) {
    ...
    Rsa.importJwkPrivateKey = function (session, jwk, algorithm, extractable, keyUsages, callback) {
        try {
            ...
            template.publicExponent = utils_1.Base64Url.decode(jwk.e);
            template.modulus = utils_1.Base64Url.decode(jwk.n);
            template.privateExponent = utils_1.Base64Url.decode(jwk.d);
            template.prime1 = utils_1.Base64Url.decode(jwk.p);
            template.prime2 = utils_1.Base64Url.decode(jwk.q);
            template.exp1 = utils_1.Base64Url.decode(jwk.dp);
            template.exp2 = utils_1.Base64Url.decode(jwk.dq);
            template.coefficient = utils_1.Base64Url.decode(jwk.qi);
            var p11key = session.create(template);
            callback(null, new key_1.CryptoKey(p11key, algorithm));
        }
        catch (e) {
            callback(e, null);
        }
            ...
            if (format === "raw") {
                ...
                session.unwrapKey(_alg, unwrappingKey.key, wrappedKey, template, function (err, p11key) {
                    if (err)
                        ...
                        callback(null, new key_1.CryptoKey(p11key, unwrappedAlgorithm));
                });
            }
            else
