...
var Ec = (function (_super) {
    ...
    Ec.exportJwkPublicKey = function (session, key, callback) {
        try {
            ...
            var jwk = {
                ...
                ext: true,
                key_ops: key.usages,
                x: utils_1.Base64Url.encode(ecPoint.x),
                y: utils_1.Base64Url.encode(ecPoint.y),
            };
            callback(null, jwk);
        }
        catch (e) {
    ...
    Ec.exportJwkPrivateKey = function (session, key, callback) {
        try {
            ...
            var jwk = {
                ...
                ext: true,
                key_ops: key.usages,
                d: utils_1.Base64Url.encode(pkey.value)
            };
            callback(null, jwk);
        }
        catch (e) {
    ...
    Ec.importJwkPrivateKey = function (session, jwk, algorithm, extractable, keyUsages, callback) {
        try {
            ...
            template.value = utils_1.Base64Url.decode(jwk.d);
    ...
    Ec.importJwkPublicKey = function (session, jwk, algorithm, extractable, keyUsages, callback) {
        try {
            ...
            template.paramsEC = namedCurve.value;
            var pointEc = EcUtils.encodePoint({ x: utils_1.Base64Url.decode(jwk.x), y: utils_1.Base64Url.decode(jwk.y) }, namedCurve);
        ...
        try {
            ...
            session.deriveKey({
                ...
                if (err)
                    ...
                    callback(null, new key_1.CryptoKey(key, derivedKeyType));
            });
        }
        catch (e) {
            callback(e, null);
