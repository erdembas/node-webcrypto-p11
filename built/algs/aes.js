...
var Aes = (function (_super) {
            ...
            session.generateKey(graphene_pk11_1.KeyGenMechanism.AES, template, function (err, key) {
                try {
                    if (err)
                        ...
                        var wcKey = new key_1.CryptoKey(key, _alg_1);
                        callback(null, wcKey);
                    }
                }
                catch (e) {
        ...
        try {
            ...
            switch (format.toLowerCase()) {
                case "jwk":
                    ...
                    var jwk = {
                        kty: "oct",
                        k: utils_1.Base64Url.encode(vals.value),
    ...
    Aes.importKey = function (session, format, keyData, algorithm, extractable, keyUsages, callback) {
        try {
            var value = void 0;
            if (format === "jwk")
                value = utils_1.Base64Url.decode(keyData.k);
            else
                value = keyData;
            var _alg = {
                name: algorithm.name,
            ...
            callback(null, new key_1.CryptoKey(sobj.toType(), _alg));
        }
        catch (e) {
            callback(e, null);
        }
